...
if __name__ == "__main__":
    ...
    def wandb_tuning():
        ...
        trainer = pl.Trainer(callbacks=ModelCheckpoint(dirpath=f"./checkpoint/{MODEL_NAME.replace('/', '_')}/{now.strftime('%Y_%m_%d_%H_%M_%S')}/", filename="{epoch}-{val_micro_f1=.2f}", monitor="val_micro_f1", mode="max"), max_epochs = wandb.config.epochs, logger=wandb_logger)
